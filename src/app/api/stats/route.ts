 ...
 *
 * A jelszó a SZAMOK_PW környezeti változóból jön; ha nincs beállítva: "enzo".
 */
...
export async function GET(request: Request) {
  const expected = process.env.SZAMOK_PW || DEFAULT_PW;

  const url = new URL(request.url);
  const given = url.searchParams.get("pw") ?? "";
  if (!passwordMatches(given, expected)) {
    return NextResponse.json({ error: "Hibás jelszó" }, { status: 401, headers: noStore });
  }

  if (!process.env.SUPABASE_URL) {
    return NextResponse.json(
      { error: "Nincs Supabase-kapcsolat (SUPABASE_URL hiányzik)." },
